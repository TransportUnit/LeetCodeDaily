...
*   Time complexity: O(m⋅n⋅logn)
    ...
    We iterate over m rows. For each row, we update the values which costs O(n). Then, we sort the row, which costs O(n⋅logn). Finally, we iterate over the row to calculate submatrix areas, which costs O(n). There is also some O(n) copying work.
